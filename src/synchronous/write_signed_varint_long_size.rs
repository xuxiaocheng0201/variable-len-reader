macro_rules! write_signed_varint_long_size {
    (cp, $func: ident, $write_internal: ident) => {
        write_signed_varint_long_size!(f cfg(feature = "sync_signed_varint_long_size"), cp, isize, $func, $write_internal);
    };
    (ap, $func: ident, $write_internal: ident) => {
        write_signed_varint_long_size!(f cfg(feature = "sync_signed_varint_long_size"), ap, isize, $func, i128, $write_internal);
    };
    (f $feature: meta, cp, $primitive: ty, $func: ident, $write_internal: ident) => {
        write_signed_varint_long!(f $feature, $primitive, $func, $write_internal);
    };
